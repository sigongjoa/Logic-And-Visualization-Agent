...
"학습의 '블랙박스'를 열어, 학생의 **역량(How)**과 **진도(What)**를 지도로 그려내는 AI 코칭 플랫폼"

기존의 교육 AI가 '정답'을 맞혔는지에 집중했다면, ATLAS는 학생이 정답에 도달하는 **'사고 과정(Thought Process)'**과 **'지식의 숙련도(Knowledge Mastery)'**를 분리하여 분석하고 시각화합니다.

본 프로젝트는 Pacer(코칭 플랫폼), Meta-RAG(논리 엔진), Manim(시각화 엔진) 프로젝트를 **'4+1축 학생 모델'**이라는 통일된 평가 기준 아래 통합하여, 코치의 티칭 활동을 극대화하는 것을 목표로 합니다.

2. 문제 정의: 학습은 '블랙박스'다

기존의 코칭 방식은 학생의 점수나 결과물에 의존합니다.

*   **코치:** 학생이 "왜" 이 문제를 틀렸는지, "어디서" 막혔는지 정확히 알기 어렵습니다. "개념은 아는 것 같은데, 왜 자꾸 계산 실수를 할까?"
...
우리의 목표: 이 블랙박스를 열어, 학생의 머릿속에 숨겨진 복잡한 인지 상태를 **'역량 지도(Capability Atlas)'**와 **'지식 지도(Knowledge Map)'**로 함께 그려내는 것입니다.

3. 핵심 솔루션: 4+1축 학생 모델

ATLAS는 학생을 **'학습 역량(How)'**을 나타내는 4축 모델과 **'학습 진도(What)'**를 나타내는 커리큘럼 축, 총 5개의 축으로 모델링합니다.

### 3.1. 축 1~4: 학습 역량 (4-Axis Capability Model)

학생의 잠재적 학습 능력을 4개의 대륙, 11개의 세부 영역으로 구성된 '인지 지도'로 모델링합니다.
...
### 3.2. 축 5: 학습 진도 (Curriculum Axis)

학생이 특정 커리큘럼의 '지식 지도'를 얼마나 정복했는지 추적합니다. 이는 단순한 점수가 아닌, '지식 그래프(Knowledge Graph)' 기반의 **개념별 숙련도(Student_Mastery)** 데이터로 관리됩니다.

*   **지식 그래프:** 커리큘럼 내의 개념들을 노드(Node)로, 개념 간의 선수 관계(e.g., '이차방정식' → '근의 공식')를 엣지(Edge)로 정의한 지도입니다.
*   **숙련도 추적:** 학생이 특정 개념(Node)에 대해 문제를 풀거나 학습할 때마다, 해당 개념의 `mastery_score`가 갱신됩니다.

4. 통합 아키텍처

ATLAS는 3개의 핵심 엔진과 1개의 중앙 DB로 구성됩니다.

*   **Pacer API (Orchestrator):** 학생/코치의 요청을 받는 메인 API 서버. Meta-RAG와 Manim을 지휘하는 오케스트레이터이자, **4+1축 모델**을 갱신하는 코칭 플랫폼의 본체입니다. [cite: sigongjoa/pacer/pacer-a60f66786b01de22ec2291ec871c4f9328d9cf73/backend/main.py]
...
*   **Platform DB (ATLAS DB):** 학생의 모든 활동, **4축 역량 모델**의 성장 이력(`Student_Vector_History`), 그리고 **커리큘럼 진도**(`Student_Mastery`)를 저장하는 핵심 자산입니다.

5. 핵심 워크플로우: 역량과 진도의 동시 측정

학생이 '근의 공식' 문제를 제출하고, 풀이 과정에서 잦은 계산 실수를 보였다고 가정합니다.

1.  **[학생]** 문제 제출 `(POST /submissions)`
2.  **[AI 분석]** Meta-RAG가 학생의 풀이를 분석합니다.
    *   **(진도 분석)** 이 문제는 '근의 공식(C-001)' 개념을 사용했음을 식별합니다.
    *   **(역량 분석)** 풀이 과정에서 계산 실수가 반복됨을 감지합니다.
3.  **[모델 동시 갱신]** AI 분석 결과를 바탕으로 두 개의 모델이 동시에 갱신됩니다.
    *   **[역량 축 갱신]** `Student_Vector_History` 테이블에 새 이력이 쌓입니다.
        *   `assessment_type`: `AI_ANALYSIS`
        *   `axis4_acc` (연산 정확성) 점수가 하락합니다.
    *   **[진도 축 갱신]** `Student_Mastery` 테이블이 갱신됩니다.
        *   `concept_id`: `C-001` (근의 공식)
        *   `mastery_score`가 50점에서 70점으로 상승합니다.
4.  **[코치 인사이트]** 코치는 대시보드에서 두 가지 차트를 동시에 봅니다.
    *   **진도 차트 (커리큘럼 축):** "좋아, 김민준 학생이 '근의 공식' 개념은 70% 숙달했군."
    *   **4축 레이더 차트 (역량 축):** "그런데 '연산 정확성'(axis4_acc) 점수가 떨어졌어. 이 학생은 개념은 아는데, 계산 실수가 잦구나."
5.  **[AI의 처방]** AI가 이 두 가지 정보를 바탕으로 코치에게 리포트를 올립니다.
    *   "김민준 학생은 '근의 공식' 개념(C-001)의 숙련도는 향상되었으나, 4축 모델의 '연산 정확성'(axis4_acc)이 하락했습니다. Anki 카드에 '근의 공식' 개념 문제 대신 '복잡한 계산 연습' 문제를 추가하는 것을 제안합니다."

이처럼 **'학습 역량(4축 모델)'**과 **'학습 진도(커리큘럼 축)'**를 분리하여 추적하면, 코치는 학생의 상태를 훨씬 더 입체적으로 분석하고 정확한 처방을 내릴 수 있습니다.

6. 개발 로드맵 (V1 vs V2)

복잡성과 배포 비용 문제를 해결하기 위해 '뼈대(V1)'를 먼저 구축하고 '엔진(V2)'을 고도화합니다.

| 구분 | V1 (현재 - 뼈대 구축) | V2 (미래 - 엔진 고도화) |
| :--- | :--- | :--- |
| Manim | 사전 제작 (콘텐츠 라이브러리)<br>(유튜브/S3에 미리 업로드) | 실시간 렌더링<br>(학생 문제에 맞춰 즉시 생성) |
| Meta-RAG | 텍스트 해설 + 개념 ID 매칭<br>(e.g., "C-001 개념 영상 보세요") | 시각화 JSON 생성<br>(Manim이 이해하는 애니메이션 설계도) |
| **4+1축 모델** | **코치 수동 갱신 / 규칙 기반**<br>(핵심 이력 데이터 축적) | **AI 자동 갱신**<br>(V1 데이터를 학습한 AI가 갱신) |
| 배포 비용 | 매우 낮음 (Cloud Run + Cloud SQL) | 높음 (실시간 GPU 렌더링 + 고성능 LLM) |

7. MLOps: AI는 코치로부터 학습한다

이 시스템의 AI는 코치(전문가)의 피드백을 통해 성장합니다.

*   **피드백 수집:** 코치가 AI의 판단(e.g., Anki 카드 생성 승인/거절)에 대해 '좋음/나쁨' 피드백을 `LLM_Log`에 기록합니다. [cite: sigongjoa/pacer/pacer-a60f66786b01de22ec2291ec871c4f9328d9cf73/backend/llm_filter.py]
...
*   `Assessments` (진단 이벤트)
*   `Student_Vector_History` (학생 잠재 공간 '이력') - **4축 역량 모델**
*   `Curriculums` (커리큘럼 정보)
*   `Concepts_Library` (개념 노드)
*   `Concept_Relations` (개념 관계: 지식 그래프)
*   `Student_Mastery` (학생별 개념 숙련도) - **커리큘럼 축**
*   `Submissions` (학생 제출물)
